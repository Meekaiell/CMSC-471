...
#               above examples.
#def pawn_promo(position):
 #   return chess.square_distance(position, (position % 8) + position)


def eval_distance_and_promotion(board):
    # running total of our evaluation of this board state
    score = 0
    tilew = 0
    tileb = 0
  #  pawnw = 0
   # pawnb = 0
    ...
    for square in chess.SQUARES:
        ...
        if piece is piece.color and chess.KING:
            tilew = square
        elif piece is not piece.color and chess.KING:
            tileb = square
        # determine if pawn is white
    #    elif piece is piece.color and chess.PAWN:
     #       pawnw = pawn_promo(square)
      #  elif piece is not piece.color and chess.PAWN:
       #     pawnb = pawn_promo(square)
        # get the distance between that piece and the center of the board
        # NOTE: 1-dist because pieces closer to the center are better
        # NOTE: the values for square are integers. They go across from the
        #       bottom-right at A1 and wrap up a row at the end. Therefore,
        #       (square % 8) is the x-coord of a square and (square / 8) is the
        #       y-coord of a square.
        if piece.color:
            k_dist = chess.square_distance(square, tilew)
        else:
            k_dist = chess.square_distance(square, tileb)
        # NOTE: we divide by 5 because that's the max distance from the middle
        ...
        if piece.color:
            dist_weight *= k_dist #* pawnw
        else:
            dist_weight *= k_dist #* pawnb
